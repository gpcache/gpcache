...
auto extractStackItemInfo(DwflSession const &dis, uintptr_t const addr)
    -> StackItem {
  StackItem result;
  ...
  if (Dwfl_Line *line_record = dwfl_module_getsrc(module, addr)) {
    ...
    result.file = dwfl_lineinfo(line_record, &dwarf_addr, &result.line, nullptr,
                                nullptr, nullptr);
  }
  return result;
}
...
void print_current_stacktrace(std::ostream &stream) {
  ...
  if (auto ex = std::current_exception()) {
    try {
      ...
      stream << "Fatal exception " << demangle(typeid(e).name()) << ": "
             << e.what() << ".\n";
    } catch (...) {
  ...
  for (int i = 0; i < stack_size; ++i) {
    auto stack_item =
        extractStackItemInfo(dis, reinterpret_cast<uintptr_t>(stack[i]));
