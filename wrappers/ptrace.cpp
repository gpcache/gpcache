...
namespace {
// Code from
// https://stackoverflow.com/questions/58320316/stdbit-cast-with-stdarray Need
// from https://en.cppreference.com/w/cpp/compiler_support ToDo: move to Utils?
template <class Dest, class Source> Dest bit_cast(Source const &source) {
...
auto readable_ptrace_request(const enum __ptrace_request request)
    -> std::string {
  switch (request) {
  case PTRACE_TRACEME:
...
auto call_ptrace(const enum __ptrace_request request, const int pid,
                 const auto addr, const auto data) {
  /* TODO:
  ...
  if constexpr (false) {
    ...
    if constexpr (std::is_pointer_v<decltype(addr)>)
      if constexpr (std::is_pointer_v<decltype(data)>)
        spdlog::debug("ptrace({}, {}, {}, {}) -> {}, {}",
                      readable_ptrace_request(request), pid,
                      reinterpret_cast<void const *>(addr),
                      reinterpret_cast<void *>(data), ret, ::strerror(errno));
      else
        spdlog::debug("ptrace({}, {}, {}, {}) -> {}, {}",
                      readable_ptrace_request(request), pid,
                      reinterpret_cast<void const *>(addr), data, ret,
                      ::strerror(errno));
    else if constexpr (std::is_pointer_v<decltype(data)>)
      spdlog::debug("ptrace({}, {}, {}, {}) -> {}, {}",
                    readable_ptrace_request(request), pid, addr,
                    reinterpret_cast<void *>(data), ret, ::strerror(errno));
    else
      spdlog::debug("ptrace({}, {}, {}, {}) -> {}, {}",
                    readable_ptrace_request(request), pid, addr, data, ret,
                    ::strerror(errno));
  }

  if (ret == -1 ||
      errno) // ToDo: improve detection. E.g. PEEKTEXT may return -1
  {
    throw fmt::format("ptrace({}, {}, {}, {}) failed: {}, {}",
                      readable_ptrace_request(request), pid, addr,
                      reinterpret_cast<void *>(data), ret, ::strerror(errno));
  }
  return ret;
...
namespace Ptrace {
namespace Raw {
void CONT(const int pid) { call_ptrace(PTRACE_CONT, pid, nullptr, 0); }
...
auto TRACEME() -> void { call_ptrace(PTRACE_TRACEME, 0, nullptr, 0); }
...
auto PEEKTEXT(const int pid, const char *const begin, const size_t count)
  ...
  for (size_t pos_in_string = 0; pos_in_string < count;
    ...
    long const data = Raw::PEEKTEXT(
        pid, reinterpret_cast<uint8_t const *>(begin) + pos_in_string);
    std::memcpy(result.data() + pos_in_string, &data,
...
auto PEEKTEXT_string(int const pid, char const *const begin) -> std::string {
  ...
  for (size_t pos_in_string = 0; pos_in_string < maximum && !end_of_string;
       pos_in_string += sizeof(intptr_t)) {
    const long data = Raw::PEEKTEXT(
        pid, reinterpret_cast<uint8_t const *>(begin) + pos_in_string);
...
auto PtraceProcess::restart_child_and_wait_for_next_syscall()
    -> std::optional<SysCall> {
  Raw::SYSCALL(pid);
...
[[nodiscard]] auto
createChildProcess(std::vector<char *> const &prog_and_arguments) -> int {
  ...
  if (pid > 0) {
    ...
    Raw::SETOPTIONS(pid, PTRACE_O_TRACESYSGOOD | PTRACE_O_TRACEFORK |
...
auto get_syscall_number_from_registers(const user_regs_struct &regs)
    -> SyscallDataType {
#if __x86_64__
...
auto get_syscall_return_value_from_registers(const user_regs_struct &regs)
    -> SyscallDataType {
#if __x86_64__
  return regs.rax;
#else
...
auto get_syscall_args(const user_regs_struct &regs)
    -> std::array<SyscallDataType, 6> {
#if __x86_64__
