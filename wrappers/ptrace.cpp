    ...
    if constexpr (false)
    {
        ...
        if constexpr (std::is_pointer_v<decltype(addr)>)
            if constexpr (std::is_pointer_v<decltype(data)>)
                spdlog::debug("ptrace({}, {}, {}, {}) -> {}, {}", readable_ptrace_request(request), pid,
                              reinterpret_cast<void const *>(addr), reinterpret_cast<void *>(data), ret,
                              ::strerror(errno));
            else
                spdlog::debug("ptrace({}, {}, {}, {}) -> {}, {}", readable_ptrace_request(request), pid,
                              reinterpret_cast<void const *>(addr), data, ret, ::strerror(errno));
        else if constexpr (std::is_pointer_v<decltype(data)>)
            spdlog::debug("ptrace({}, {}, {}, {}) -> {}, {}", readable_ptrace_request(request), pid, addr,
                          reinterpret_cast<void *>(data), ret, ::strerror(errno));
        else
            spdlog::debug("ptrace({}, {}, {}, {}) -> {}, {}", readable_ptrace_request(request), pid, addr, data, ret,
                          ::strerror(errno));
    }

    if (ret == -1 || errno) // ToDo: improve detection. E.g. PEEKTEXT may return -1
    {
        throw fmt::format("ptrace({}, {}, {}, {}) failed: {}, {}", readable_ptrace_request(request), pid, addr,
                          reinterpret_cast<void *>(data), ret, ::strerror(errno));
    }
    return ret;
}
...
[[nodiscard]] auto createChildProcess(std::vector<char *> const &prog_and_arguments) -> int
{
    ...
    if (pid > 0)
    {
        ...
        Raw::SETOPTIONS(pid, PTRACE_O_TRACESYSGOOD | PTRACE_O_TRACEFORK | PTRACE_O_TRACEVFORK | PTRACE_O_TRACEEXEC |
                                 PTRACE_O_TRACECLONE);
