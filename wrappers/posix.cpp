...
auto waitpid(const int pid, const int options) -> StopReason {
  ...
  if (WIFEXITED(status))
    return {.state = StopReason::ProcessState::EXITED,
            .signal_number = {},
            .exit_status = WEXITSTATUS(status)};

  if (WIFSIGNALED(status))
    return {.state = StopReason::ProcessState::EXITED_BECAUSE_OF_SIGNAL,
            .signal_number = WTERMSIG(status)};

  if (WIFSTOPPED(status))
    return {.state = StopReason::ProcessState::STOPPED_BECAUSE_OF_SIGNAL,
...
auto wait_for_signal2(int const pid, std::vector<int> const signum_to_wait_for)
    -> bool {
  ...
  case StopReason::ProcessState::STOPPED_BECAUSE_OF_SIGNAL:
    if (!gpcache::contains(signum_to_wait_for, status.signal_number))
      throw fmt::format("Child is stopped for other signal: {}",
                        status.signal_number.value());
    // fallthrough
  }
