...
template <class T> auto PEEKTEXT(int pid, const T *ptr_in_pid) -> T {
  T result;
  auto data =
      PEEKTEXT(pid, reinterpret_cast<const char *>(ptr_in_pid), sizeof(T));
...
auto get_readable_param_value(auto pid, auto param_type, auto param_value) {
  if (param_type == std::string("const char *") ||
      param_type == std::string("char *")) {
    std::string str = Ptrace::PEEKTEXT_string(
        pid, reinterpret_cast<char const *>(param_value));
    ...
    if (syscall._cached_string_representation.empty()) {
      const std::string params = [&]() {
        ...
        for (auto [pos, param] : enumerate(syscall.info.params)) {
          ...
          auto param_value = get_readable_param_value(syscall.pid, param.type,
                                                      syscall.arguments[pos]);
          params +=
              fmt::format("{} {} = {}", param.type, param.name, param_value);
        }
        return params;
      }();

      syscall._cached_string_representation = fmt::format(
          "{}({}) --> {}", syscall.info.name, params,
