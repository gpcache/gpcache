...
auto execute_cached_syscall(State &, CachedSyscall_Read::Parameters const &cached_syscall) -> CachedSyscall_Read::Result
{
    ...
    {
        result.return_value =
            pread64(cached_syscall.fd, result.data.data(), cached_syscall.count, cached_syscall.pread64_offset);
    }
    else
    {
...
auto covert_real_to_cachable_syscall(State &, Syscall_read const &syscall) -> CachedSyscall_Read
{
    std::string const data = Ptrace::PEEKTEXT(syscall.pid, syscall.buf(), syscall.count());
    CachedSyscall_Read::Parameters const parameters{(int)syscall.fd(), syscall.count(), false, 0};
    CachedSyscall_Read::Result const result{hash_of_data(data), (int)syscall.return_value(), syscall.errno_value()};
    return {parameters, result};
}

auto covert_real_to_cachable_syscall(State &, Syscall_pread64 const &syscall) -> CachedSyscall_Read
{
    std::string const data = Ptrace::PEEKTEXT(syscall.pid, syscall.buf(), syscall.count());
    CachedSyscall_Read::Parameters const parameters{(int)syscall.fd(), syscall.count(), true, syscall.pos()};
    CachedSyscall_Read::Result const result{hash_of_data(data), (int)syscall.return_value(), syscall.errno_value()};
    return {parameters, result};
}
} // namespace gpcache
