...
namespace gpcache
{
auto execute_cached_syscall(State &, CachedSyscall_Write::Parameters const &cached_syscall)
...
auto covert_real_to_cachable_syscall(State &, Syscall_write const &syscall) -> CachedSyscall_Write
{
    std::string const data = Ptrace::PEEKTEXT(syscall.pid, syscall.buf(), syscall.count());
    CachedSyscall_Write::Parameters const parameters{(int)syscall.fd(),
                                                     data}; // ToDo: binary data... store as separate file?
