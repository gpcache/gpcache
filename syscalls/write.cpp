...
auto covert_to_cachable_syscall(State &, Syscall_write const &syscall)
  ...
  CachedSyscall_Write::Parameters const parameters{
      (int)syscall.fd(), data}; // ToDo: binary data... store as separate file?
  CachedSyscall_Write::Result const result{(int)syscall.return_value(),
                                           syscall.errno_value()};
