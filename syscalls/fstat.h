...
{
    return int_to_strong_order(
        std::memcmp(reinterpret_cast<const void *>(&lhs), reinterpret_cast<const void *>(&rhs), sizeof(struct stat)));
}
...
template <> struct fmt::formatter<struct stat>
{
    ...
    auto format(struct stat const &s, auto &ctx)
    {
        return fmt::format_to(ctx.out(),
                              ...
                              "size {}, blksize {}, blocks {}, "
                              "atim {}.{}, mtim {}.{}, ctim {}.{})",
                              s.st_dev, s.st_ino, s.st_mode, s.st_nlink, s.st_uid, s.st_gid, s.st_rdev, s.st_size,
                              s.st_blksize, s.st_blocks, s.st_atim.tv_sec, s.st_atim.tv_nsec, s.st_mtim.tv_sec,
                              s.st_mtim.tv_nsec, s.st_ctim.tv_sec, s.st_ctim.tv_nsec);
    }
};
NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE(struct stat, st_dev, st_ino, st_mode, st_nlink, st_uid, st_gid, st_rdev, st_size,
                                   st_blksize, st_blocks, st_atim.tv_sec, st_atim.tv_nsec, st_mtim.tv_sec,
                                   st_mtim.tv_nsec, st_ctim.tv_sec, st_ctim.tv_nsec)
...
auto covert_real_to_cachable_syscall(State &state, Syscall_fstat const &syscall) -> CachedSyscall_Fstat;
} // namespace gpcache
