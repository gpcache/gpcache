...
inline auto operator<=>(const struct stat &lhs, const struct stat &rhs) {
  return int_to_strong_order(std::memcmp(reinterpret_cast<const void *>(&lhs),
...
NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE(struct stat, st_dev, st_ino, st_mode,
                                   st_nlink, st_uid, st_gid, st_rdev, st_size,
                                   st_blksize, st_blocks, st_atim.tv_sec,
                                   st_atim.tv_nsec, st_mtim.tv_sec,
                                   st_mtim.tv_nsec, st_ctim.tv_sec,
                                   st_ctim.tv_nsec)
