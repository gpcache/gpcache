...
auto execute_cached_syscall(State &state, CachedSyscall_Open::Parameters const &cached_syscall)
...
{
    {
        ...
        if (path.is_absolute() && (syscall.flags() == O_CLOEXEC || syscall.flags() == (O_RDONLY | O_CLOEXEC) ||
                                   syscall.flags() == (O_RDONLY | O_LARGEFILE)))
        {
            CachedSyscall_Open const cached_syscall{{0, path, syscall.flags(), syscall.mode()},
