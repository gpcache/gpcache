...
auto execute_cached_syscall(
    State &state, CachedSyscall_Open::Parameters const &cached_syscall)
    -> CachedSyscall_Open::Result {
  if (!gpcache::contains(mode_allowlist, cached_syscall.mode)) {
    spdlog::warn(
        "Cannot execute {} from cache at the moment (not yet implemented)",
  ...
  int const fd = openat(0, cached_syscall.filename.c_str(),
                        cached_syscall.flags, cached_syscall.mode);
  result.fd = fd; // todo: fd is dynamic
  result.errno_code = errno;

  if (fd >= 0)
    state.fds.open(fd, cached_syscall.filename, cached_syscall.flags,
                   "cached syscall");
  return result;
}
...
auto covert_to_cachable_syscall(State &state, Syscall_openat const &syscall)
    -> std::optional<CachedSyscall_Open> {
  {
    ...
    if (path.is_absolute() && (syscall.flags() == O_CLOEXEC ||
                               syscall.flags() == (O_RDONLY | O_CLOEXEC) ||
                               syscall.flags() == (O_RDONLY | O_LARGEFILE))) {
      CachedSyscall_Open const cached_syscall{
          {0, path, syscall.flags(), syscall.mode()},
          {(int)syscall.return_value(), syscall.errno_value()}};
      if (syscall.errno_value() == 0)
        state.fds.open(syscall.return_value(), path, syscall.flags(),
                       json(cached_syscall).dump());
      ...
      spdlog::warn("flags {} = {}", syscall.flags(),
                   openat_flag_to_string(syscall.flags()));
      return {};
    }
  }
}
