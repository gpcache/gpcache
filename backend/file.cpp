...
static auto read_file_to_json(const std::filesystem::path &path)
    -> std::variant<json, int> {
...
static auto write_file(const std::filesystem::path &path,
                       const std::string &content) -> std::variant<bool, int> {
  // todo: exact effect of binary
...
static auto is_file_content(const std::filesystem::path &file,
                            const std::string &content)
    -> is_file_content_result {
...
static auto ensure_file_content(const std::filesystem::path &file,
                                const std::string &content)
  ...
  if (std::filesystem::exists(file)) {
    ...
    if (std::holds_alternative<std::string>(res)) {
      ...
      spdlog::warn("Cannot read cached file {} because of {}", file.string(),
                   existing_file_error);
...
auto cache_input(const std::filesystem::path &path,
                 const std::string &input_name, const json &parameters,
                 json result, std::vector<std::string> const &sloppiness) {
  ...
  const json action_file_content = {{"syscall_name", input_name},
                                    {"parameters", parameters}};

  auto const action_file = path / "next_syscall.txt";
  auto const result_file = result_path / "readable_result.txt";

  if (auto action_match =
          ensure_file_content(action_file, action_file_content.dump());
      action_match.ok()) {
    if (auto result_match = ensure_file_content(result_file, result.dump());
        result_match.ok())
      return result_path;
    else
    ...
    if (action_match.old_file_content) {
      spdlog::warn(
          "Same application is suddenly performing a different syscall for no "
...
auto FileBasedBackend::store(json const &executable_and_params,
                             std::vector<CachedSyscall> const &syscalls,
                             std::vector<std::string> const &sloppiness)
    -> void {
  auto directory = this->cache_path;
  directory = cache_input(directory, "params", json(), executable_and_params,
                          sloppiness);
  for (const CachedSyscall &syscall : syscalls) {
    std::visit(
        [&directory, &sloppiness](auto &&typed_syscall) {
          directory = cache_input(directory, typed_syscall.name,
                                  json(typed_syscall.parameters),
...
auto FileBasedBackend::get_all_possible_results(
    const std::filesystem::path &pos) -> std::vector<json> {
  ...
  for (auto const &result_path : std::filesystem::directory_iterator(pos)) {
    ...
    if (json const *const data = std::get_if<json>(&content)) {
      ...
      spdlog::warn("Error reading possible result {}: {}", result_file.string(),
                   std::get<int>(content));
    }
  }
  return result;
