...
auto FiledescriptorState::dump_data(auto const level,
                                    FiledescriptorData const &data) const
    -> void {
  ...
  spdlog::log(level, "file descriptor flags: {}",
              openat_flag_to_string(data.flags));
...
auto FiledescriptorState::dump(auto const level,
                               file_descriptor_t const fd) const -> void {
...
auto FiledescriptorState::dump(spdlog::level::level_enum const level) const
    -> void {
...
auto FiledescriptorState::get_open(file_descriptor_t const fd) const
    -> const FiledescriptorData & {
  if (auto entry = fds.find(fd);
      entry != fds.end() && entry->second.state == State::open) {
    return entry->second;
  } else {
...
auto FiledescriptorState::get_open_opt(file_descriptor_t const fd) const
    -> std::optional<FiledescriptorData> {
  if (auto entry = fds.find(fd);
      entry != fds.end() && entry->second.state == State::open) {
...
auto FiledescriptorState::open(file_descriptor_t fd, std::string file,
                               int flags, std::string source) -> void {
  FiledescriptorData new_entry = {.fd = fd,
                                  .filename = file,
                                  .flags = flags,
                                  .state = State::open,
...
auto FiledescriptorState::close(file_descriptor_t fd, std::string source)
    -> void {
  if (auto entry = fds.find(fd); entry != fds.end()) {
    auto &data = entry->second;
    if (data.state == State::closed) {
      throw std::runtime_error(
          fmt::format("closing closed fd {} from {}", fd, source));
    } else {
    ...
    throw std::runtime_error(
        fmt::format("closing unknown fd {} from {}", fd, source));
  }
}
} // namespace gpcache
