#include "cache_executed_syscall.h"
...
// ToDo: consider adding CachedSyscall_Unsupported with bool flag or
// CachedSyscall_Unsupported and CachedSyscall_Ignore
...
auto handle_syscall(Ptrace::PtraceProcess const p, Ptrace::SysCall const &ptrace_syscall, State &state) -> SyscallResult
{
    Syscall_Base syscall{
        .pid = p.get_pid(), .args = ptrace_syscall.arguments, .real_return_value = ptrace_syscall.return_value.value()};

    // Design problem:
    // * variant with all possible syscall types takes minutes to compile
    // * creating hundreds of virtual methods is kind of pointless when most of
    // them are not supported So the only solution is this huge switch statement.
    switch (ptrace_syscall.info.syscall_id)
    {
    case Syscall_brk::syscall_id:
    case Syscall_arch_prctl::syscall_id:
    case Syscall_mprotect::syscall_id: // maybe interesting with ignoring more
                                       // mmap calls... ignore for now
        return SyscallResult{true};
    case Syscall_access::syscall_id: {
        return covert_real_to_cachable_syscall(state, static_cast<Syscall_access>(syscall));
    }
    case Syscall_openat::syscall_id: {
        auto const cached_syscall = covert_real_to_cachable_syscall(state, static_cast<Syscall_openat>(syscall));
    ...
    case Syscall_close::syscall_id: {
        return covert_real_to_cachable_syscall(state, static_cast<Syscall_close>(syscall));
    }
    case Syscall_fstat::syscall_id: {
        return covert_real_to_cachable_syscall(state, static_cast<Syscall_fstat>(syscall));
    }
    case Syscall_read::syscall_id: {
        return covert_real_to_cachable_syscall(state, static_cast<Syscall_read>(syscall));
    }
    case Syscall_pread64::syscall_id: {
        return covert_real_to_cachable_syscall(state, static_cast<Syscall_pread64>(syscall));
    }
    case Syscall_write::syscall_id: {
        return covert_real_to_cachable_syscall(state, static_cast<Syscall_write>(syscall));
    }
    case Syscall_mmap::syscall_id: {
        // Wow.. C++... In caes of 'a' return 'a', in case of 'b' return 'b'.
        auto result = covert_real_to_cachable_syscall(state, static_cast<Syscall_mmap>(syscall));
...
auto execute_program(std::vector<char *> const &prog_and_arguments) -> std::vector<CachedSyscall>
{
    ...
    while (true)
    {
        ...
        // Since gpcache does not modify syscalls, just monitoring the exists is
        // sufficient
