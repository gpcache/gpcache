...
auto handle_syscall(Ptrace::PtraceProcess const p,
                    Ptrace::SysCall const &ptrace_syscall, State &state)
    -> SyscallResult {
  Syscall_Base syscall{.pid = p.get_pid(),
                       .args = ptrace_syscall.arguments,
                       .real_return_value =
                           ptrace_syscall.return_value.value()};
...
auto execute_program(std::vector<char *> const &prog_and_arguments)
    -> std::vector<CachedSyscall> {
  ...
  while (true) {
    ...
    if (const CachedSyscall *const cached_syscall =
            std::get_if<CachedSyscall>(&result)) {
