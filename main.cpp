...
auto json_cached_syscall_to_string(const json &cached_syscall) {
  return cached_syscall.at("syscall_name").get<std::string>() +
         cached_syscall.at("parameters").dump();
}
...
int main(int argc, char **argv) {
  ...
  absl::SetProgramUsageMessage(
      "General Purpose Cache will speed up repetitios retesting, just as "
  ...
  spdlog::set_level(absl::GetFlag(FLAGS_verbose) ? spdlog::level::debug
                                                 : spdlog::level::info);
  ...
  if (params.empty()) {
    fmt::print(
        "Pass your executable to gpcache, e.g. gpcache echo 'Hello, World!'\n");
    exit(1);
  }
  ...
  try {
    ...
    if (cached.ok()) {
      ...
      spdlog::info("Great, {} is cached. Now let's check all dependencies...",
                   json(params).dump());
      gpcache::State state;

      while (cached.next_syscall) {
        auto execution_result = gpcache::execute_cached_json_syscall(
        ...
        if (new_cached.ok()) {
          spdlog::info(
              "Cached {} -> Real {} -> Cache Hit",
              json_cached_syscall_to_string(cached.next_syscall.value()),
          ...
          spdlog::warn(
              "Cached {} -> Real {} -> Cache MISS",
              json_cached_syscall_to_string(cached.next_syscall.value()),
              execution_result.dump());
          for (auto const &possible_result :
               backend.get_all_possible_results(cached.path))
            spdlog::warn("Cached results would have been: {}",
                         possible_result.dump());
        }
    ...
    params.push_back(
        nullptr); // required for syscalls so end of array can be detected.

    std::vector<gpcache::CachedSyscall> execution_cache =
        gpcache::execute_program(params);
